...
public class StatCalc {
    ...
    public void itemRecycled(Recyclable recyclable, double weight) {
        ...
        rcmDataList.add(new rcmData(timestamp, recyclable.getType(), weight, recyclable.getPricePerKg()));
    }
    ...
    public Rewards totalMoneyIssued() {
         double total = 0;
        for (rcmData RCMData: rcmDataList)
            total += RCMData.getPrice() * RCMData.getWeight();
        return new Rewards(0, (int)total);
    }

    public Rewards getRewardsInTimeFrame(int timeFrame) {
        double total = 0;
